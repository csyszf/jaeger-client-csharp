...
namespace Jaeger.Senders
{
    ...
    /// </summary>
    public class TestUdpSender : ThriftSender
    {
        ...
        public TestUdpSender(string host, int port, int maxPacketSize)
        ...
        protected override async Task SendAsync(ThriftProcess process, List<ThriftSpan> spans, CancellationToken cancellationToken)
        {
            try
            {
                ...
                cnt += spans.Count;
                // Interlocked.Add(ref cnt, spans.Count);
            }
            catch (Exception ex)
            {
